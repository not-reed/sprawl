...
export async function processMemoryForGraph(
  ...
  if (embeddingOpts?.apiKey) {
    ...
    if (failed.length > 0) {
      logger?.warning(`Failed to generate embeddings for ${failed.length}/${nodeEntries.length} nodes`)
    }
  }
  ...
  logger?.info(`Graph: extracted ${result.entities.length} entities, ${result.relationships.length} relationships from memory [${memoryId}]`)
  return result
}
