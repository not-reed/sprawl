...
export async function observe(
  ...
  if (isDegenerateRaw(text)) {
    toolLog.warning`Degenerate observer response detected (length=${String(text.length)}), discarding`
    return { observations: [] }
  }
  ...
  try {
    ...
    toolLog.warning`Failed to parse observer response (length=${String(text.length)}): ${text.slice(0, 150)}…${text.slice(-100)}`
    return { observations: [] }
  }
