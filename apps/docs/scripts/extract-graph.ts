...
async function main() {
  ...
  const model = process.env.MEMORY_WORKER_MODEL ?? process.env.OPENROUTER_MODEL ?? 'google/gemini-3.1-flash-lite-preview'
  ...
  const nodesArray = Array.from(nodes.values()).map(({ appearsIn, ...rest }) => rest)
