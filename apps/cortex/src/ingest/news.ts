...
export async function fetchCryptoPanicNews(
  ...
  if (!res.ok) {
    throw new IngestError(`CryptoPanic error: ${res.status}`);
  }
...
export async function fetchCryptoPanicHistorical(
  ...
  for (let page = 0; page < pages; page++) {
    const url = nextUrl ?? `${CRYPTOPANIC_BASE}/posts/?auth_token=${apiKey}&kind=news&public=true`;
...
export async function fetchCryptoCompareNews(apiKey?: string): Promise<NewsData[]> {
  ...
  const res = await fetch(url);
  if (!res.ok) {
    throw new IngestError(`CryptoCompare error: ${res.status}`);
  }
...
function assertJsonResponse(res: Response, source: string): void {
  if (!isJsonResponse(res)) {
    throw new IngestError(
      `${source} returned non-JSON content-type: ${res.headers.get("content-type")}`,
    );
  }
}
