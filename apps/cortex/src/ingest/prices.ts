  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (res.ok) {
      ...
      if (!ct.includes("application/json")) {
        throw new IngestError(`CoinGecko returned non-JSON (${ct}), likely Cloudflare challenge`);
      }
      return res;
    }
    ...
    throw new IngestError(`CoinGecko error: ${res.status}`);
  }
  throw new IngestError("CoinGecko: exhausted retries");
}
