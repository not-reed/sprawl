...
export async function getActiveTokens(db: DB): Promise<TrackedToken[]> {
  return db.selectFrom("tracked_tokens").selectAll().where("active", "=", 1).execute();
}
...
export async function insertNewsItem(
  ...
  const existing = await db
    ...
    .select("id")
    .where((eb) => eb.or([eb("external_id", "=", item.external_id), eb("title", "=", item.title)]))
    .executeTakeFirst();
...
export async function linkNewsToMemory(db: DB, newsId: string, memoryId: string): Promise<void> {
  await db
...
export async function insertCommand(db: DB, command: string, args?: string): Promise<string> {
  const id = nanoid();
  await db
    .insertInto("commands")
