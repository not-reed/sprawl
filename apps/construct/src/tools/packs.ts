...
import {
  createSecretStoreTool,
  createSecretListTool,
  createSecretDeleteTool,
} from "./core/secret-manage.js";
...
export const TOOL_PACKS: ToolPack[] = [
  {
    ...
    factories: [
      ...
      (ctx) =>
        createMemoryRecallTool(ctx.db, ctx.apiKey, ctx.embeddingModel) as InternalTool<TSchema>,
      ...
      (ctx) =>
        ctx.extensionsDir
          ? (createIdentityReadTool(ctx.extensionsDir) as InternalTool<TSchema>)
          : null,
      (ctx) =>
        ctx.extensionsDir
  ...
  },
  {
    ...
    factories: [
      ...
      (ctx) =>
        ctx.tavilyApiKey ? (createWebSearchTool(ctx.tavilyApiKey) as InternalTool<TSchema>) : null,
    ],
  },
  {
    name: "self",
  ...
  },
  {
    ...
    factories: [
      (ctx) =>
        ctx.telegram ? (createTelegramReactTool(ctx.telegram) as InternalTool<TSchema>) : null,
      (ctx) =>
        ctx.telegram ? (createTelegramReplyToTool(ctx.telegram) as InternalTool<TSchema>) : null,
      (ctx) =>
        ctx.telegram ? (createTelegramPinTool(ctx.telegram) as InternalTool<TSchema>) : null,
      (ctx) =>
        ctx.telegram ? (createTelegramUnpinTool(ctx.telegram) as InternalTool<TSchema>) : null,
      (ctx) =>
        ctx.telegram ? (createTelegramGetPinnedTool(ctx.telegram) as InternalTool<TSchema>) : null,
      (ctx) =>
        ctx.telegram
          ? (createTelegramAskTool(ctx.db, ctx.telegram) as InternalTool<TSchema>)
          : null,
    ],
  },
];
...
/**
 * Select packs via embedding similarity, then instantiate all tools from matched packs.
 * @param queryEmbedding - Embedding of the user's message. Undefined causes all packs to load.
 * @param ctx - Shared context (db, API keys, telegram handle, etc.) passed to tool factories.
 */
