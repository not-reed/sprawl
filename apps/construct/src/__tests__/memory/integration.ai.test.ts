...
async function setupDb(): Promise<Kysely<Database>> {
  ...
  const d = result.db;
  await migration001.up(d as Kysely<any>);
  await migration002.up(d as Kysely<any>);
  await migration004.up(d as Kysely<any>);
  await migration005.up(d as Kysely<any>);
  await migration006.up(d as Kysely<any>);
  return d;
}
...
describeIntegration("extractEntities", () => {
...
describeIntegration("processMemoryForGraph", () => {
...
describeIntegration("MemoryManager.runObserver - basic pipeline", () => {
...
describeIntegration("MemoryManager.runObserver - batching", () => {
