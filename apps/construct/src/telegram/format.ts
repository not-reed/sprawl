...
export function markdownToTelegramHtml(md: string): string {
  ...
  text = text.replace(
    /^#{1,6}\s+(.+)$/gm,
    (_, content) => `<b>${content.replace(/\*\*/g, "")}</b>`,
  );
  ...
  // Restore code blocks and inline code (NUL-byte placeholders)
  text = text.replace(
    /\x00CB(\d+)\x00/g,
    (_, i) => `<pre>${escapeHtml(codeBlocks[parseInt(i)])}</pre>`,
  );
  text = text.replace(
    /\x00IC(\d+)\x00/g,
    (_, i) => `<code>${escapeHtml(inlineCodes[parseInt(i)])}</code>`,
  );
