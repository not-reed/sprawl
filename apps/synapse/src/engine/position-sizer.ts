...
export type SizeResult = { viable: true; sizeUsd: number } | { viable: false; reason: string };
...
export function computePositionSize(
  ...
  if (gasPct > config.maxGasPct) {
    return {
      viable: false,
      reason: `gas_too_expensive:${gasPct.toFixed(1)}%>${config.maxGasPct}%`,
    };
  }
  ...
  if (sizeUsd > availableCashUsd) {
    return {
      viable: false,
      reason: `exceeds_cash:${sizeUsd.toFixed(2)}>${availableCashUsd.toFixed(2)}`,
    };
  }
