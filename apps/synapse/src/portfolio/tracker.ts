 ...
 */
export async function updatePositionPrices(db: DB, prices: Map<string, number>): Promise<void> {
...
export async function recalculatePortfolio(db: DB): Promise<void> {
  ...
  const positions = await getOpenPositions(db);
  const positionsValue = positions.reduce((sum, p) => sum + p.current_price_usd * p.quantity, 0);
