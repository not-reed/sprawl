...
import { createTestPosition, createTestPortfolioState } from "./fixtures.js";
...
const makePosition = createTestPosition;
const makePortfolioState = createTestPortfolioState;
...
describe("checkPortfolioRisk", () => {
  ...
  it("triggers halt at max drawdown", () => {
    // 15% drawdown
    expect(checkPortfolioRisk(8500, 10000, config)).toEqual({
      safe: false,
  ...
  it("triggers halt beyond max drawdown", () => {
    expect(checkPortfolioRisk(8000, 10000, config)).toEqual({
      safe: false,
      reason: "drawdown_halt",
    });
  });
