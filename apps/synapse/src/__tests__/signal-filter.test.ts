...
import { createTestSignal } from "./fixtures.js";
...
const makeSignal = createTestSignal;
...
describe("filterSignal", () => {
  ...
  it("passes sell with position and low confidence (0.3)", () => {
    const result = filterSignal(makeSignal({ signal_type: "sell", confidence: 0.3 }), config, true);
    expect(result.pass).toBe(true);
  });

  it("rejects sell with position below 0.3 confidence", () => {
    const result = filterSignal(makeSignal({ signal_type: "sell", confidence: 0.2 }), config, true);
    expect(result).toEqual({ pass: false, reason: "low_confidence" });
  });

  it("rejects short-term buy below 0.4 confidence", () => {
    const result = filterSignal(makeSignal({ confidence: 0.3, timeframe: "short" }), config, false);
    expect(result).toEqual({ pass: false, reason: "low_confidence" });
  });

  it("rejects long-term buy below 0.6 confidence", () => {
    const result = filterSignal(makeSignal({ confidence: 0.5, timeframe: "long" }), config, false);
    expect(result).toEqual({ pass: false, reason: "low_confidence" });
  });

  it("passes long-term buy at 0.6 confidence", () => {
    const result = filterSignal(makeSignal({ confidence: 0.6, timeframe: "long" }), config, false);
    expect(result.pass).toBe(true);
  });
