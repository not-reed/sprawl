...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("opened_at", "text", (col) => col.notNull().defaultTo(sql`(datetime('now'))`))
    .addColumn("closed_at", "text")
    .execute();

  await db.schema.createIndex("idx_positions_token").on("positions").column("token_id").execute();

  await db.schema.createIndex("idx_positions_open").on("positions").column("closed_at").execute();

  // trades
  await db.schema
    .createTable("trades")
    .addColumn("id", "text", (col) => col.primaryKey())
    .addColumn("position_id", "text", (col) => col.notNull().references("positions.id"))
    .addColumn("signal_id", "text", (col) => col.notNull())
    .addColumn("token_id", "text", (col) => col.notNull())
  ...
  await db.schema.createIndex("idx_trades_position").on("trades").column("position_id").execute();
  ...
  await db.schema
    ...
    .addColumn("processed_at", "text", (col) => col.notNull().defaultTo(sql`(datetime('now'))`))
    .execute();
