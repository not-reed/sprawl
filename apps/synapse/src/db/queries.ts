...
export async function getOpenPositionByToken(db: DB, tokenId: string): Promise<Position | undefined> {
  return db
    .selectFrom('positions')
    .selectAll()
...
export async function getLatestSnapshot(db: DB): Promise<import('./schema.js').Snapshot | undefined> {
  return db
...
export async function getRecentRiskEvents(db: DB, limit = 20): Promise<import('./schema.js').RiskEvent[]> {
  return db
