...
const totalReturn = ((state.total_value_usd - env.INITIAL_BALANCE_USD) / env.INITIAL_BALANCE_USD) * 100
...
if (trades.length > 0) {
  for (const t of trades) {
    ...
    console.log(`  ${ts}  ${dir} ${t.token_symbol}  $${t.price_usd.toFixed(4)}  size $${t.size_usd.toFixed(2)}`)
  }
} else {
  console.log('  (none)')
}
