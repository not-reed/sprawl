...
export const api = {
  ...
  getMemory: (id: string) =>
    fetchJson<import('./types.js').Memory>(`/memories/${id}`),
  ...
  getNode: (id: string) =>
    fetchJson<import('./types.js').GraphNode>(`/graph/nodes/${id}`),

  getNodeEdges: (id: string) =>
    fetchJson<{ edges: import('./types.js').GraphEdge[] }>(
      `/graph/nodes/${id}/edges`,
    ),

  traverseNode: (id: string, depth = 2) =>
    fetchJson<{ nodes: (import('./types.js').GraphNode & { depth: number; via_relation: string | null })[] }>(
      `/graph/nodes/${id}/traverse?depth=${depth}`,
    ),

  getNodeMemories: (id: string) =>
    fetchJson<{ memories: import('./types.js').Memory[] }>(
      `/graph/nodes/${id}/memories`,
    ),

  getFullGraph: (limit = 200) =>
    fetchJson<import('./types.js').GraphData>(
      `/graph/full?limit=${limit}`,
    ),
