...
export function GraphView() {
  ...
  const render = useCallback(() => {
    ...
    for (const link of graph.links) {
      ...
      if (
        !graph.typeFilters.has(source.node_type) ||
        !graph.typeFilters.has(target.node_type)
      )
        continue

      const isSelected =
        graph.selectedNodeId === source.id ||
        graph.selectedNodeId === target.id

      ctx.strokeStyle = isSelected
        ? 'rgba(124, 108, 240, 0.4)'
        : 'rgba(60, 60, 80, 0.3)'
      ctx.lineWidth = isSelected
        ? (1 + link.weight * 0.3) / t.k
        : (0.5 + link.weight * 0.15) / t.k
  ...
  const allNodeNames = new Map(
    graph.nodes.map((n) => [
      n.id,
      { display_name: n.display_name, node_type: n.node_type },
    ]),
  )

  if (graph.loading) {
    return <div className="loading" style={{ flex: 1 }}>Loading graph...</div>
  }
