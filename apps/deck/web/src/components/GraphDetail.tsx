    ...
    <div className="graph-detail">
      <div className="graph-detail-header">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div className="graph-detail-name" style={{ color }}>
              {node.display_name}
            </div>
      ...
      {edges.length > 0 && (
        <div className="graph-detail-section">
          ...
          {edges.map((edge) => {
            ...
            return (
              <div key={edge.id} className="graph-edge-item" onClick={() => onSelectNode(otherId)}>
                <span
                  style={{
                    width: 6,
                    height: 6,
                    borderRadius: "50%",
                    background: other ? getNodeColor(other.node_type) : "var(--gray)",
                    flexShrink: 0,
                  }}
                />
                <span>{other?.display_name ?? otherId}</span>
                <span className="graph-edge-relation">{edge.relation}</span>
                <span className="graph-edge-weight">w:{edge.weight}</span>
              </div>
            );
          })}
        </div>
