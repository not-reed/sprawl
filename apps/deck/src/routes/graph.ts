...
import { searchNodes, getNodeEdges, traverseGraph, getRelatedMemoryIds } from "@repo/cairn";
...
graphRoutes.get("/nodes/search", async (c) => {
  ...
  const cleaned = nodes.map(({ embedding: _embedding, ...rest }) => rest);
...
graphRoutes.get("/nodes/:id/traverse", async (c) => {
  const db = c.get("db");
  const id = c.req.param("id");
  const maxDepth = Math.min(Number(c.req.query("depth") ?? 2), 4);

  const results = await traverseGraph(db, id, maxDepth);

  const nodes = results.map(
    ({ node: { embedding: _embedding, ...rest }, depth, via_relation }) => ({
      ...rest,
      depth,
      via_relation,
    }),
...
graphRoutes.get("/full", async (c) => {
  ...
  if (nodeIds.length > 0) {
    edges = await db
      ...
      .selectAll()
      .where((eb) => eb.and([eb("source_id", "in", nodeIds), eb("target_id", "in", nodeIds)]))
      .execute();
  }

  const cleanedNodes = nodes.map(({ embedding: _embedding, ...rest }) => rest);
