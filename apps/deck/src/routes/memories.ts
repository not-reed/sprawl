...
import { recallMemories, getRecentMemories, getMemoryNodes, generateEmbedding } from "@repo/cairn";
...
memoriesRoutes.get("/search", async (c) => {
  ...
  if ((mode === "auto" || mode === "embedding") && env.OPENROUTER_API_KEY) {
    try {
      queryEmbedding = await generateEmbedding(env.OPENROUTER_API_KEY, q, env.EMBEDDING_MODEL);
    } catch {
