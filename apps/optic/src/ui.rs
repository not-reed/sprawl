use crate::db::{
    ChartData, CommandRow, CortexDb, GraphRow, NewsRow, PortfolioRow, PositionRow, PriceRow,
    RiskEventRow, SignalLogRow, SignalRow, Stats, SynapseDb, TradeRow,
};
...
impl TradingPanel {
    pub fn next(self) -> Self {
        match self {
            TradingPanel::Positions => TradingPanel::Trades,
            TradingPanel::Trades => TradingPanel::SignalLog,
            TradingPanel::SignalLog => TradingPanel::RiskEvents,
            TradingPanel::RiskEvents => TradingPanel::Positions,
        }
    }
}
...
pub struct AppState {
    ...
    // Trading view
    pub trading_focused: TradingPanel,
    pub trading_scroll_offsets: [usize; 4],
    pub portfolio: Option<PortfolioRow>,
    pub positions: Vec<PositionRow>,
    pub trades: Vec<TradeRow>,
    pub signal_log: Vec<SignalLogRow>,
    pub risk_events: Vec<RiskEventRow>,
}

impl AppState {
    pub fn new(synapse_connected: bool) -> Self {
        Self {
            ...
            recent_commands: vec![],
            trading_focused: TradingPanel::Positions,
            trading_scroll_offsets: [0; 4],
            portfolio: None,
            positions: vec![],
            trades: vec![],
            signal_log: vec![],
    ...
    pub fn refresh(&mut self, db: &CortexDb, synapse: Option<&SynapseDb>) {
        ...
        if let Some(sdb) = synapse {
            self.portfolio = sdb.get_portfolio().unwrap_or(None);
            self.positions = sdb.get_open_positions().unwrap_or_default();
            self.trades = sdb.get_recent_trades(50).unwrap_or_default();
            self.signal_log = sdb.get_recent_signal_log(50).unwrap_or_default();
            self.risk_events = sdb.get_recent_risk_events(50).unwrap_or_default();
        }
    ...
    pub fn scroll_up(&mut self) {
        if self.mode == ViewMode::Trading {
            let idx = self.trading_focused as usize;
            self.trading_scroll_offsets[idx] =
                self.trading_scroll_offsets[idx].saturating_sub(1);
            return;
        }
        match self.focused {
    ...
    pub fn scroll_down(&mut self) {
        if self.mode == ViewMode::Trading {
            let idx = self.trading_focused as usize;
            self.trading_scroll_offsets[idx] += 1;
            return;
        }
        match self.focused {
            Panel::News => {
    ...
    pub fn cycle_focus(&mut self) {
        if self.mode == ViewMode::Trading {
            self.trading_focused = self.trading_focused.next();
        } else {
...
pub fn draw(frame: &mut Frame, state: &AppState) {
    ...
    let has_portfolio_bar = state.synapse_connected && state.portfolio.is_some();

    // Main layout: body + optional portfolio bar + status bar
    let mut constraints = vec![Constraint::Min(3)];
    if has_portfolio_bar {
    ...
    let body_area = main[0];
    let status_idx = main.len() - 1;

    match state.mode {
        ViewMode::Market => draw_market_view(frame, body_area, state),
        ViewMode::Trading => draw_trading_view(frame, body_area, state),
    }

    if has_portfolio_bar {
        draw_portfolio_bar(frame, main[status_idx - 1], state);
    }

    draw_status(frame, main[status_idx], state);
...
fn draw_market_view(frame: &mut Frame, area: Rect, state: &AppState) {
...
fn draw_trading_view(frame: &mut Frame, area: Rect, state: &AppState) {
    // 2x2 grid
    ...
    draw_positions(frame, top[0], state);
    draw_trades(frame, top[1], state);
    draw_signal_log(frame, bottom[0], state);
    draw_risk_events(frame, bottom[1], state);
}
...
fn trading_panel_border(panel: TradingPanel, current: TradingPanel) -> Style {
    if panel == current {
        Style::default()
            .fg(trading_panel_color(panel))
            .add_modifier(Modifier::BOLD)
    } else {
        Style::default().fg(Color::DarkGray)
    }
...
fn trading_panel_color(panel: TradingPanel) -> Color {
    match panel {
        TradingPanel::Positions => Color::Cyan,
        TradingPanel::Trades => Color::Yellow,
        TradingPanel::SignalLog => Color::Blue,
        TradingPanel::RiskEvents => Color::Red,
    }
}

fn draw_portfolio_bar(frame: &mut Frame, area: Rect, state: &AppState) {
    let p = match &state.portfolio {
        Some(p) => p,
        None => return,
    };

    let return_pct = if p.high_water_mark_usd > 0.0 {
        (p.total_value_usd - p.high_water_mark_usd) / p.high_water_mark_usd * 100.0
    } else {
        0.0
    };
    let (ret_str, ret_color) = format_pct(Some(return_pct));
    let dd_color = if p.drawdown_pct > 5.0 {
        Color::Red
    } else if p.drawdown_pct > 2.0 {
        Color::Yellow
    } else {
        Color::Green
    };

    let halted_span = if p.halted {
        Span::styled(" HALTED ", Style::default().fg(Color::White).bg(Color::Red).add_modifier(Modifier::BOLD))
    } else {
    ...
    let spans = vec![
        ...
        Span::styled(
            format_usd(p.total_value_usd),
            Style::default().fg(Color::White).add_modifier(Modifier::BOLD),
        ),
        Span::styled("  Cash ", Style::default().fg(Color::DarkGray)),
        Span::styled(format_usd(p.cash_usd), Style::default().fg(Color::White)),
        Span::styled("  DD ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{:.1}%", p.drawdown_pct), Style::default().fg(dd_color)),
        Span::styled("  Ret ", Style::default().fg(Color::DarkGray)),
        Span::styled(ret_str, Style::default().fg(ret_color)),
        Span::raw("  "),
        halted_span,
        Span::styled(
            format!("  {}", time_ago(&p.updated_at)),
            Style::default().fg(Color::DarkGray),
        ),
    ];
...
fn draw_positions(frame: &mut Frame, area: Rect, state: &AppState) {
    let offset = state.trading_scroll_offsets[TradingPanel::Positions as usize];

    let header = Row::new(["Symbol", "Entry", "Current", "Size", "uPnL", "PnL%", "SL", "TP"])
        .style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD));
    ...
    let rows: Vec<Row> = state
        ...
        .skip(offset)
        .map(|p| {
            let pnl_pct = if p.size_usd > 0.0 {
                p.unrealized_pnl_usd / p.size_usd * 100.0
            } else {
                0.0
            };
            let pnl_color = if p.unrealized_pnl_usd >= 0.0 {
                Color::Green
            } else {
                Color::Red
            };
            let dir_indicator = if p.direction == "short" { "S " } else { "" };
            Row::new([
                Cell::from(format!("{}{}", dir_indicator, p.token_symbol)).style(Style::default().bold()),
                Cell::from(format_usd(p.entry_price_usd)),
                Cell::from(format_usd(p.current_price_usd)),
                Cell::from(format_usd(p.size_usd)),
                Cell::from(format!("{:+.2}", p.unrealized_pnl_usd)).style(Style::default().fg(pnl_color)),
                Cell::from(format!("{:+.1}%", pnl_pct)).style(Style::default().fg(pnl_color)),
                Cell::from(format_usd(p.stop_loss_price)).style(Style::default().fg(Color::Red)),
                Cell::from(format_usd(p.take_profit_price)).style(Style::default().fg(Color::Green)),
            ])
        })
        .collect();

    let count = state.positions.len();
    let title = format!(" Positions ({}) ", count);
    let table = Table::new(
        rows,
        [
            Constraint::Length(8),
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(trading_panel_border(TradingPanel::Positions, state.trading_focused))
            .title(title),
    );
...
fn draw_trades(frame: &mut Frame, area: Rect, state: &AppState) {
    let offset = state.trading_scroll_offsets[TradingPanel::Trades as usize];
    ...
    let items: Vec<ListItem> = state
        ...
        .skip(offset)
        .map(|t| {
            let dir_color = if t.direction == "buy" {
                Color::Green
            } else {
                Color::Red
            };
            let line = Line::from(vec![
                Span::styled(
                    format!("{:>4} ", time_ago(&t.executed_at)),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(
                    format!("{:<4}", t.direction.to_uppercase()),
                    Style::default().fg(dir_color).add_modifier(Modifier::BOLD),
                ),
                Span::styled(
                    format!("{:<6}", t.token_symbol),
                    Style::default().bold(),
                ),
                Span::styled(format_usd(t.price_usd), Style::default().fg(Color::White)),
                Span::styled(
                    format!("  {}", format_usd(t.size_usd)),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(
                    format!("  gas:{}", format_usd(t.gas_usd)),
                    Style::default().fg(Color::DarkGray),
                ),
            ]);
            ListItem::new(line)
    ...
    let count = state.trades.len();
    let title = format!(" Trades ({}) ", count);
    let list = List::new(items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(trading_panel_border(TradingPanel::Trades, state.trading_focused))
            .title(title),
    );
    frame.render_widget(list, area);
}

fn draw_signal_log(frame: &mut Frame, area: Rect, state: &AppState) {
    let offset = state.trading_scroll_offsets[TradingPanel::SignalLog as usize];
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|s| {
            let action_color = match s.action.as_str() {
                "opened_position" => Color::Green,
                "closed_position" => Color::Yellow,
                "skipped" => Color::DarkGray,
                _ => Color::White,
            };
            let conf_pct = format!("{:.0}%", s.confidence * 100.0);
            let reason = s
                .skip_reason
                .as_deref()
                .map(|r| format!(" ({})", truncate_str(r, 30)))
                .unwrap_or_default();
            let line = Line::from(vec![
                Span::styled(
                    format!("{:>4} ", time_ago(&s.processed_at)),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(
                    format!("{:<6}", s.token_id),
                ...
                Span::styled(format!("{:>4} ", conf_pct), Style::default().fg(Color::White)),
                Span::styled(
                    s.action.clone(),
    ...
    let count = state.signal_log.len();
    let title = format!(" Signal Log ({}) ", count);
    let list = List::new(items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(trading_panel_border(TradingPanel::SignalLog, state.trading_focused))
            .title(title),
    );
    frame.render_widget(list, area);
}

fn draw_risk_events(frame: &mut Frame, area: Rect, state: &AppState) {
    let offset = state.trading_scroll_offsets[TradingPanel::RiskEvents as usize];
    ...
    let items: Vec<ListItem> = state
        ...
        .skip(offset)
        .map(|e| {
            let severity_color = match e.event_type.as_str() {
                "drawdown_halt" => Color::Red,
                "stop_loss" => Color::Yellow,
                "exposure_limit" => Color::Yellow,
                "take_profit" => Color::Green,
                _ => Color::White,
            };
            let line = Line::from(vec![
                Span::styled(
                    format!("{:>4} ", time_ago(&e.created_at)),
                    Style::default().fg(Color::DarkGray),
                ),
                Span::styled(
                    format!("{:<16}", e.event_type),
                    Style::default().fg(severity_color).add_modifier(Modifier::BOLD),
                ),
                Span::styled(
                    truncate_str(&e.details, 60).to_string(),
                    Style::default().fg(Color::White),
                ),
            ]);
            ListItem::new(line)
    ...
    let count = state.risk_events.len();
    let title = format!(" Risk Events ({}) ", count);
    let list = List::new(items).block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(trading_panel_border(TradingPanel::RiskEvents, state.trading_focused))
            .title(title),
    );
    frame.render_widget(list, area);
}
...
fn draw_status(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    if state.synapse_connected {
        let mode_label = match state.mode {
            ViewMode::Market => "[Market]",
    ...
    let keybinds = match state.mode {
        ViewMode::Market => " | q:quit r:refresh Tab:focus j/k:scroll c:chart a:analyze 1/2:mode",
        ViewMode::Trading => " | q:quit r:refresh Tab:focus j/k:scroll 1/2:mode",
    };
    let keybinds = if state.synapse_connected {
        keybinds
    } else {
