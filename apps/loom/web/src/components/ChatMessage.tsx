...
export function ChatMessage({ message, audioUrl: initialAudioUrl, onGenerateAudio }: ChatMessageProps) {
    ...
    <div className={`chat-message ${isUser ? 'chat-message-user' : 'chat-message-gm'}`}>
      ...
      {!isUser && audioUrl && (
        <div className="audio-controls">
          <button className="audio-btn" onClick={() => skip(-5)} title="Back 5s">{'\u23EA'}</button>
          <button className="audio-btn audio-btn-play" onClick={togglePlay} title={playing ? 'Pause' : 'Play'}>
            {playing ? '\u23F8' : '\u25B6'}
          </button>
          <button className="audio-btn" onClick={() => skip(5)} title="Forward 5s">{'\u23E9'}</button>
          <input
            type="range"
