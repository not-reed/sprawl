...
export async function scriptify(text: string, db: Kysely<Database>): Promise<Segment[] | null> {
  ...
  console.log(
    `[scriptify] starting, model=${model}, input=${text.length} chars, system_prompt=${SYSTEM_PROMPT.length} chars`,
  );
  ...
  try {
    ...
    console.log(
      `[scriptify] fetch=${fetchMs}ms, parse=${parseMs}ms, usage=${JSON.stringify(data.usage)}`,
    );
    ...
    console.log(
      `[scriptify] parsed ${segments.length} segments:`,
      segments.map((s) => `[${s.speaker}] ${s.text.slice(0, 60)}...`),
    );
    return segments;
  } catch (err) {
