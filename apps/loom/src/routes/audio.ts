...
async function getVoiceConfig(
  db: Kysely<Database>,
): Promise<{ defaultVoice: string; npcVoices: Record<string, string> }> {
  try {
    const row = await db
      .selectFrom("settings")
      .select("value")
...
export async function generateAudio(
  ...
  try {
    ...
    if (env.TTS_SCRIPTIFY) {
      ...
      console.log(
        `[tts] scriptify: ${elapsed(scriptStart)} → ${segments ? segments.length + " segments" : "null (fallback)"}`,
      );
    ...
    console.log(
      `[tts] generateAudio done in ${elapsed(t0)}, cached as ${id} (${combined.length} bytes)`,
    );
    return id;
  } catch (err) {
...
export async function streamAudioResponse(
  ...
  if (env.TTS_SCRIPTIFY) {
    ...
    const segments = await scriptify(text, db);
    console.log(
      `[tts:stream] scriptify: ${elapsed(scriptStart)} → ${segments ? segments.length + " segments" : "null (fallback)"}`,
    );
