...
settingsRoutes.post("/voice-preview", async (c) => {
  ...
  const sample =
    text?.trim() ||
    "You enter the tavern. A fire crackles in the hearth, and the innkeeper looks up with a crooked smile.";
...
settingsRoutes.get("/characters", async (c) => {
  ...
  try {
    ...
    for (const n of nodes) {
      ...
      const capitalized = n.name.replace(/\b\w/g, (ch) => ch.toUpperCase());
  ...
  return c.json({ characters: [...names].toSorted() });
});
